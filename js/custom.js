document.addEventListener('DOMContentLoaded', function() {
  ...
  async function fetchGitHubProfile() {
    ...
    const profileHTML = `
      <div class="Box">
        <img src="${data.avatar_url}" class="avatar avatar-3" alt="...">
        <div class="Box-body">
    ...
    repos.forEach(repo => {
      reposHTML += `
        <div class="col-md-6 mb-4">
          <div class="Box">
            <div class="Box-body">
              <h5 class="Box-title">${repo.name}</h5>
  ...
  async function fetchCommitActivity(repoName) {
    const response = await fetch(`https://api.github.com/repos/${username}/${repoName}/stats/participation`);
    const data = await response.json();
